...
static void usage() {
    log::out << "usage: artic [options] files...\n"
                ...
                "         --emit-c-interface     Emits C interface from Artic code\n"
...
static bool compile(const ProgramOptions& opts, Log& log) {
    ...
    if (opts.emit_thorin)
        world.dump();
    if (opts.emit_cint) {
        size_t pos = opts.module_name.find_last_of("\\/");
        pos = (pos == std::string::npos) ? 0 : pos + 1;
        auto guard = opts.module_name.substr(pos) + ".h";
        ...
        if (!file)
            ...
            file << "/* " << name << ": Artic interface file generated by thorin */" << std::endl;
            file << "#ifndef " << guard << std::endl;
            file << "#define " << guard << std::endl << std::endl;
            file << "#ifdef __cplusplus" << std::endl;
            file << "extern \"C\" {" << std::endl;
            file << "#endif" << std::endl << std::endl;

            thorin::emit_cint(world, file);
